...
/*版本说明
  v2.3:修改配置页面信息，修改厂家参数需要输入密码。20170412  zdf
...
QiaqiaMQTTSD::QiaqiaMQTTSD() {
    ...
    //button
    button_first_state = 0;
    button_timer_begin = 0;
}
...
void QiaqiaMQTTSD::sdInit(void){
    ...
    digitalWrite(ledPin, LOW);  //LED引脚置低，不亮
    //button  end
    */
	sdDataMemoryInit();
	...
	if(readFileFlag == 0){
        ...
        if(SerFlash.open("/storage/configinfo.bin", FS_MODE_OPEN_READ) != SL_FS_OK){
          ...
          Serial.println("please use APMode to config parameters");
            ...
            SerFlash.close();
            readFileFlag = 1;  //再次循环时不需要再读文件数据了
        }
    }
    ...
    while(1)  //相当于void loop(){}
    {
        ...
        if(IsLoop == 0){
            if((millis() - apConfigStart) > 120000){  //访问AP模式的时间窗口是120s，超过
                ...
                delay(1000);
                //PRCMMCUReset(true);  //reset MCU (包括外设)
                PRCMSOCReset();
            }
        }   
        ...
        if (packetSize)
        {
            ...
            unsigned char llen = sizeof(SlNetCfgIpV4Args_t);
            sl_NetCfgGet(SL_IPV4_STA_P2P_CL_GET_INFO, NULL, &llen, (unsigned char*)&IpAddr);   
            uint8_t addr_3 = ((IpAddr.ipV4 >> (3*8)) & 0xFF);
            uint8_t addr_2 = ((IpAddr.ipV4 >> (2*8)) & 0xFF);
            uint8_t addr_1 = ((IpAddr.ipV4 >> (1*8)) & 0xFF);
            uint8_t addr_0 = ((IpAddr.ipV4 >> (0*8)) & 0xFF);
            sprintf(IP_Addr, "%d.%d.%d.%d", addr_3,addr_2,addr_1,addr_0);
            ...
            if((packetBuffer[0] == 'I') && (packetBuffer[1] == 'P')){
                Udp.beginPacket(Udp.remoteIP(), Udp.remotePort());
                Udp.write(IP_Addr);
                //Udp.write("IP address = 192.168.1.1");
                Udp.endPacket();
            }
                    ...
                    if (c == '\n'){    // if the byte is a newline character
                        ...
                        if (strlen(buffer) == 0)
                        {
                            ...
                            if (resetflag == 1) {
                                ...
                                /*
                                myClient.println("iptxt.onblur= function(){");
                                myClient.println("var cloudip = iptxt.value;var reg = /^\d+(\.)/g;var re = new RegExp(reg);");
                                myClient.println("if(cloudip.length>15||!re.test(cloudip)){msg.innerHTML = '* cloudIp 输入小于等于24位的小写字母和数字的组合';");
                                myClient.println("}else{ msg.innerHTML = '';;}}");
                                */
                    ...
                    if(text.endsWith("HTTP/1.")){
                        if(text.startsWith("GET /A")){ 
                            ...
                            if(retval == SL_FS_OK){
                                ...
                                //SerFlash.freeString();   //某些例程中有这条语句
                                SerFlash.close();
                                //readfileflag = 0;  //因为重启了设备，所以不用标记了
...
bool QiaqiaMQTTSD::sdWifiConfig(void){
    ...
    return false;  //两种方式都没成功，返回false
}
/*
void QiaqiaMQTTGW::buttonAPTest(void){
    if(digitalRead(button_SW2) == 1){
        Serial.println("---button has been pushed---");

        if(button_first_state == 1){
          button_timer_begin = millis();
          //Serial.println(button_timer_begin);
          button_first_state = 0;
        }
      
        if((millis() - button_timer_begin) < 0){
            Serial.println("millis() runoff");  //按下时正赶上溢出
            button_first_state = 1;  //下次重新标记按下时刻
            //button_timer_begin = 0; 
        }

        if((millis() - button_timer_begin) > 4000){
            digitalWrite(ledPin, HIGH);  //点亮一个灯，指示进入AP模式
            gwAPModeConfig();
        }
    }else{
        button_first_state = 1;//重新标记按下时刻
    }
...
bool QiaqiaMQTTSD::sdRun() {
	...
	if (WiFi.status() == WL_CONNECTED) {
		if (pClient && !pClient->connected()) {
			SD_DBG("Disconnected Reconnecting....");
...
int QiaqiaMQTTSD::sdAckuseridRegist() {
	SD_DBG("enter sdAckuseridRegist");
...
void QiaqiaMQTTSD::sdPayloadAckuseridParse(byte *payload, unsigned int length) {
	SD_DBG("enter sdPayloadAckuseridParse");
	...
	//const char *devindex = root["subdevs"][2]["devindex"];   ????????????????
...
/*
int QiaqiaMQTTGW::online(int busport, int devaddr, bool onlinestate) {
	SD_DBG("enter online");
...
sdWriteCallbackParameters QiaqiaMQTTSD::sdWriteCallbackParametersDecode(byte *payload, unsigned int length) {
	SD_DBG("enter sdWriteCallbackParametersDecode");
...
sdReadCallbackParameters QiaqiaMQTTSD::sdReadCallbackParametersDecode(byte *payload, unsigned int length) {
  	SD_DBG("enter sdReadCallbackParametersDecode");
	...
	const char *sflag_read = root["devid"]["sflag"];   //payload中的信息是源设备的。
...
/*
//zdf  第一版升级信息，payload中只有完整的升级路径，需自己拆分出各个信息。
void QiaqiaMQTTGW::gwUpdateParametersDecode(byte *payload, unsigned int length){
	SD_DBG("enter gwUpdateParametersDecode");
	...
	const char *value_update = root["data"]["value"];  //把升级路径提取出来
	SD_DBG("enter gwUpdate path Parse");
	uint16_t value_len = strlen(value_update);  //路径信息总长度
	...
	int len = 0;
	char *pfrom = strstr(value_update, "/");  //http:
	if(pfrom != NULL)
		strncpy(m_sdUpdateParameters.updateHeader, (char *)value_update, (uint8_t)(pfrom - value_update));
	if(strlen(m_sdUpdateParameters.updateHeader) == 0) return;
	len += (strlen(m_sdUpdateParameters.updateHeader) + 2);  //有两个"//"
	SD_DBG(m_sdUpdateParameters.updateHeader);


	char *pIP = strstr(pfrom + 2, ":"); 
  	if(pIP != NULL)
    	strncpy(m_sdUpdateParameters.updateIP, (char *)pfrom + 2, (uint8_t)(pIP - pfrom - 2));
  	if(strlen(m_sdUpdateParameters.updateIP) == 0) return;
  	len += (strlen(m_sdUpdateParameters.updateIP) + 1);
  	SD_DBG(m_sdUpdateParameters.updateIP);

  	char *pPort = strstr(pIP + 1, "/");
  	if(pPort != NULL)
    	strncpy(m_sdUpdateParameters.updatePort, (char *)pIP + 1, (uint8_t)(pPort - pIP - 1));
  	if(strlen(m_sdUpdateParameters.updatePort) == 0) return;
  	len += (strlen(m_sdUpdateParameters.updatePort) + 1);
  	SD_DBG(m_sdUpdateParameters.updatePort);

  	char *pPath = pPort;
  	if((pPath != NULL) && (len < value_len))
    	strncpy(m_sdUpdateParameters.updatePath, (char *)pPort, (size_t)(value_len -len + 1));
  	if(strlen(m_sdUpdateParameters.updatePath) == 0) return;
  	SD_DBG(m_sdUpdateParameters.updatePath);
}
*/

//第二版升级信息，payload中既有完整的路径信息，也有拆分好的各部分信息，可直接使用。
void QiaqiaMQTTSD::sdUpdateParametersDecode(byte *payload, unsigned int length){
	SD_DBG("enter sdUpdateParametersDecode");
    	...
    	if (err >= 0){
    		Serial.print("Got status code: ");
      		...
      		if (err >= 0){
      			...
      			int body_onepart = bodyLen/4; //zdf
      			Serial.print("Content length is: ");
        		Serial.println(bodyLen);
        		Serial.println();
        		Serial.println("Body returned follows:");

				sdUpdateStatus("0"); //zdf
	...
	case MQTT_MSG_TYPE_READ:
		if (m_SdInfo.sd_sub_successed == true) {
			if (atol(m_topicParseInfo.str_offsetindex) <= DATA_FUN_POINT) {
				...
				case FUN_MSG_TYPE_READ_TIME:   //后台暂无此命令，待测试！20161229
					/*
                    {
					m_sdReadCallbackParameters = sdReadCallbackParametersDecode(payload, length);
                    /*
				...
				case FUN_MSG_TYPE_UPDATE_STATUS_F:  //unused
					break;
				case FUN_MSG_TYPE_UPDATE_STATUS_S:  //unused
					break;
		...
		if (m_SdInfo.sd_sub_successed == true) {
			if (atol(m_topicParseInfo.str_offsetindex) <= DATA_FUN_POINT) {
				...
				case FUN_MSG_TYPE_DEV_RESTART:
					Serial.println("enter singledevice restart callback");
    				delay(1000);
				...
				case FUN_MSG_TYPE_WIFI_CLEAR:
                {
					...
					//delay(50);
					//gwWiFiClear();  //清除WIFI并重启设备，进入smartconfig
					/*
					int timeout_smart = 0,
					if(WiFi.startSmartConfig(false) == 0) {
        				while (WiFi.status() != WL_CONNECTED) {
							Serial.print(".");
    						delay(1000);
    						if(timeout_smart++ == 45){
    							Serial.println("smartconfig wifi timeout");
    							//return false;
    							break;
    						}
    					}
        			//Serial.println("smartconfig wifi succeed");
        			//return true;
    				}
    				*/
					break;
...
/*
void QiaqiaMQTTSD::setGwAndDevTimeCallback(settimecallback outersettimecall) {
	m_SetTimeCallback = outersettimecall;
}

void QiaqiaMQTTSD::setGwAndDevRestartCallback(setrestartcallback outersetrestartcall) {
...
/*
//zdf
void QiaqiaMQTTGW::gwWiFiClear(){
	Serial.println("enter wifi clear");
    sl_WlanProfileDel(WLAN_DEL_ALL_PROFILES);  //清除存储的wifi名及密码
    delay(50);
    PRCMSOCReset();
}
*/
