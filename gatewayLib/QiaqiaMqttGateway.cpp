...
#include <string.h>  //文找到该头文件，功能待确定

//v2.0.0: 初始版本
//v2.0.1：修改参数配置页面格式，前四个参数需输入密码修改。20170412
...
void qiaqiaMqttGateway::gwInit() {
    ...
    */
	...
	/*生成可变AP热点名时使用
	WiFi.macAddress(mac);
    sprintf(MAC, "%x:%x:%x", mac[3],mac[4],mac[5]);  //wifi模块mac地址后六位
    strcat(AP_NAME, MAC);
    */
    ...
    if(WiFi.status() == WL_CONNECTED) {
        if(pClient && !pClient->connected()){
            ...
            if(!pClient->connect(clientID, m_GwInfo.gwindex, m_GwInfo.apikey)) {
                ...
                if((millis() - RemoteConfigStart) > 5000){  //5s内连接云端server失败，进入AP配置参数
                    GW_DBG_ln("enter APMode to config wifi parameters");
                    gwAPModeConfig();
                }
            } else {
...
void qiaqiaMqttGateway::gwAPModeConfig() {
    ...
    */
    ...
    while(1)  //相当于void loop(){}
    {
        ...
        if(IsLoop == 0){
            if((millis() - apConfigStart) > 60000){  //访问AP模式的时间窗口是60s
                //Udp.stop();
...
void qiaqiaMqttGateway::gwPayloadAckuseridParse(byte *payload, unsigned int length) {
    ...
    */
    ...
    //const char *devindex = root["subdevs"][2]["devindex"];  //有用吗？？
...
void qiaqiaMqttGateway::buttonAPTest(){
    if(digitalRead(button_SW2) == 1){
        GW_DBG_ln("---button has been pushed---");
...
/*
void qiaqiaMqttGateway::gwWiFiClear(){
    Serial.println("enter wifi clear");
